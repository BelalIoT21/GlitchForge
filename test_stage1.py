...
try:
    ...
    print("✓ Base scanner imported successfully")
except Exception as e:
    print(f"✗ Failed to import base scanner: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"⚠ Import error (this is OK if config.py is missing): {e}")
    print("  The scanners will still work individually")
except Exception as e:
    print(f"✗ Failed to import main orchestrator: {e}")
...
try:
    import os
    if not os.path.exists('config.py'):
        print("⚠ Skipping - config.py needed for this test")
    else:
        import config
    ...
    print(f"⚠ Scanner initialization issue: {e}")
...
try:
    import os
    if not os.path.exists('config.py'):
        print("⚠ Skipping - config.py needed")
    else:
        import config
        sql_scanner = SQLInjectionScanner(config.SCANNER_CONFIG)
        xss_scanner = XSSScanner(config.SCANNER_CONFIG)
    ...
    print(f"⚠ Payload loading issue: {e}")
...
try:
    ...
    print(f"⚠ Metrics calculator issue: {e}")
...
try:
    ...
    if missing_dirs:
        print(f"⚠ Some directories are missing: {', '.join(missing_dirs)}")
...
import os
if os.path.exists('config.py'):
    print("\n✓ All tests passed - Stage 1 is ready!")
    print("\nNext steps:")
    ...
    print("\n⚠ IMPORTANT: config.py is missing!")
